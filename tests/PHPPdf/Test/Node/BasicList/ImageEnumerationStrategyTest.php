...
class ImageEnumerationStrategyTest extends \PHPPdf\PHPUnit\Framework\TestCase
{
    ...
    public function drawEnumerationInValidPosition(Point $point, $position, $childMarginLeft, $fontSize)
    {
        ...
        $listMock = $this->getMock('PHPPdf\Node\BasicList', array('getChild', 'getAttribute', 'getRecurseAttribute', 'getImage'));
    ...
    {
        $elementIndex = 1;
        $listMock = $this->getMock('PHPPdf\Node\BasicList', array('getImage', 'getChild'));
        
        $listMock->expects($this->once())
                 ->method('getImage')
                 ->will($this->returnValue(null));
                 
        $child = $this->getMock('PHPPdf\Node\Container', array());
