...
class RowTest extends TestCase
{
    ...
    {
        $tableMock = $this->getMock('PHPPdf\Node\Table', array(
            'getRowHeight'
        ));
    ...
    public function setNumberOfColumnForCells(array $colspans)
    {
        ...
        {
            $cell = $this->getMock('PHPPdf\Node\Table\Cell', array('setNumberOfColumn', 'getColspan'));
...
//                array(2, 1),
    ...
    {
        $cell = $this->getMock('PHPPdf\Node\Table\Cell', array('addListener'));
    ...
    private function createMockedCellsWithHeights(array $heights)
    {
        ...
        {
            $cell = $this->getMock('PHPPdf\Node\Table\Cell', array('getHeight'));
            $cell->expects($this->atLeastOnce())
                 ->method('getHeight')
    ...
    private function createMockedCellsWidthVerticalMargins($marginsTop, $marginsBottom)
    {
        ...
        {
            $cell = $this->getMock('PHPPdf\Node\Table\Cell', array('getMarginTop', 'getMarginBottom'));
            $cell->expects($this->atLeastOnce())
                 ->method('getMarginTop')
